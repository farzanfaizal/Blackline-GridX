...
def parse_car_details_and_part_name(combined_text):
    """
    Parse the combined 'CAR DETAILS AND PART NAME' field to extract:
    ...
    prompt_text = f"""Parse this automotive parts listing text and extract the car details and part name separately.

Text: {text}

Extract:
1. Car Details: Make, model, and year range (if available)
2. Part Name: The actual part description/name

Return in this exact format:
CAR_DETAILS: [extracted car details]
PART_NAME: [extracted part name]

Rules:
- Car details should include make, model, and year if present
- Part name should be the functional description of the part
- Remove "GENUINE USED" from part name if present
- Clean up any formatting issues or extra characters"""
    ...
    if not car_details or not part_name:
        ...
        if year_match:
            ...
            # Extract text before the year as car details
            car_part = text[:year_match.start()].strip()
            # Extract text after the year as part description
            part_part = text[year_match.end():].strip()
            
            car_details = f"{car_part} {year_part}".strip()
            part_name = re.sub(r'\bGENUINE USED\b', '', part_part).strip()
        else:
            # Simple split by first number sequence that looks like a part number
            part_num_match = re.search(r'[A-Z0-9]{6,}', text)
            if part_num_match:
                car_details = text[:part_num_match.start()].strip()
                part_name = text[part_num_match.end():].strip()
                part_name = re.sub(r'\bGENUINE USED\b', '', part_name).strip()
            else:
                # Last resort: split roughly in half
                mid = len(text) // 2
                car_details = text[:mid].strip()
                part_name = text[mid:].strip()
...
def generate_title_with_length_control(car_details, part_no, part_name, sku):
    """Generate title with strict 79-character limit enforcement - NO PIPES"""
    identifier = part_no if part_no else sku
    
    # Progressive shortening prompts - REMOVED ALL PIPES
    prompts = [
        # First attempt - full format with spaces and dashes
        f"""Generate an SEO-friendly eBay title EXACTLY 79 characters or less (including all spaces and punctuation) in this format:
[Car Details] [Year Range] [Part Name] Used Genuine Good Condition [Part No]

Car Details: {car_details}
Part Name: {part_name} (First Letter Caps like normal)
Part Number: {identifier} (Exact Same, Full CAPS)

Do not Miss any details from these {car_details}, {part_name} and {identifier}
Return ONLY the title with no explanation:""",
        
        # Second attempt - remove "Good Condition"
        f"""Generate an SEO-friendly eBay title EXACTLY 79 characters or less (including all spaces and punctuation) in this format:
[Car Details] [Year Range] [Part Name] Used Genuine [Part No]
...
Return ONLY the title with no explanation:""",
        ...
        # Third attempt - shorter format
        f"""Generate an SEO-friendly eBay title EXACTLY 79 characters or less (including all spaces and punctuation) in this format:
[Car Details] [Part Name] Used [Part No]
...
Return ONLY the title with no explanation:""",
        ...
        # Fourth attempt - minimal format
        f"""Generate an SEO-friendly eBay title EXACTLY 79 characters or less (including all spaces and punctuation):
[Car Details] [Part Name] Used [Part No]
...
Return ONLY the title with no explanation:"""
    ...
    for attempt, prompt_text in enumerate(prompts, 1):
        logger.info(f"   Title generation attempt {attempt}/4")
        ...
        if title:
            ...
            # Check if title meets length requirement
            if len(title) <= 79:
                logger.info(f"   ✅ Title generated successfully: {len(title)} characters")
                return title
            else:
                logger.warning(f"   ⚠️ Title too long: {len(title)} characters, trying shorter format...")
    
    # Fallback: Manual truncation as last resort
    logger.warning("   ⚠️ All attempts failed, using manual truncation...")
    fallback_title = f"{car_details[:30]} - {part_name[:25]} - {identifier}"
    if len(fallback_title) > 79:
        fallback_title = fallback_title[:76] + "..."
...
def process_row(row, idx, total):
    ...
    logger.info(f"   📄 Parsed - Car: '{car_details}' | Part: '{part_name}'")

    # Use new title generation function with length control
    title = generate_title_with_length_control(car_details, part_no_fallback, part_name, sku)
...
def process_csv():
    input_file = r"B-4_BLK_70_Input.csv"
    output_file = r"B-4_BLK_70_Output.csv"
